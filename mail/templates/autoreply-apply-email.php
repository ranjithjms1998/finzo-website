<?php
/**
 * Auto-reply email — sent back to the person who submitted the Apply Now
 * form, confirming receipt. $data keys: name, mobile, email, loanType,
 * loanAmount, employmentType, city, message.
 */

require_once __DIR__ . '/layout.php';
require_once __DIR__ . '/notification-apply-email.php'; // for finzo_format_inr()

if (!function_exists('finzo_apply_autoreply_email_html')) {
    function finzo_apply_autoreply_email_html(array $config, array $data): string
    {
        ...
        $body = <<<HTML
<span style="display:inline-block; background-color:rgba(212,175,55,0.12); border:1px solid rgba(212,175,55,0.4); color:#b9922c; font-family:Arial, sans-serif; font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; padding:6px 14px; border-radius:50px; margin-bottom:16px;">Application Received</span>
...
<p style="margin:0 0 24px; font-family:Arial, sans-serif; font-size:14px; color:#5a6478; line-height:1.6;">
  We've received your {$loanType} enquiry and a member of the Finzo Financial Services team will review your requirement and get in touch with you shortly. For your records, here's a copy of what you submitted:
</p>
...
<p style="margin:24px 0 0; font-family:Arial, sans-serif; font-size:12px; color:#8a95ad; line-height:1.6;">
  This is an automated acknowledgement — submission of an enquiry does not guarantee loan approval, a specific interest rate, tenure or disbursal. Final terms are subject to the policies and approval process of the respective lender. If you did not submit this enquiry, you can safely ignore this email or contact us at {$companyEmail}.
</p>
HTML;

        return finzo_email_shell($config, "Thanks for your {$loanType} enquiry — Finzo Financial Services", $body);
    }
}
